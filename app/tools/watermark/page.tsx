  ...
  return (
    <div style={{ minHeight: "100vh", padding: "32px" }}>
      <div style={{ marginBottom: "28px" }}>
        ...
        <h1 style={{ fontSize: "28px", fontWeight: 700, color: "white", letterSpacing: "-0.02em" }}>Add Watermark</h1>
        <p style={{ color: "rgba(255,255,255,0.4)", marginTop: "6px", fontSize: "14px" }}>Live preview updates as you change settings.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: "20px", minHeight: "calc(100vh - 200px)" }}>
        <div className="glass-card" style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "16px", overflowY: "auto" }}>
          {!file ? (
            ...
            <div style={{ padding: "10px 12px", borderRadius: "10px", background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.2)", fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>
          ...
          {tab === "text" && (
            <>
              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "6px" }}>Watermark Text</label>
                <input className="input-field" value={text} onChange={e => setText(e.target.value)} placeholder="CONFIDENTIAL" />
              </div>

              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "6px" }}>Font Size: {fontSize}pt</label>
                <input type="range" min={12} max={120} value={fontSize} onChange={e => setFontSize(Number(e.target.value))} />
              </div>

              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "6px" }}>Opacity: {Math.round(opacity * 100)}%</label>
                <input type="range" min={5} max={100} value={opacity * 100} onChange={e => setOpacity(Number(e.target.value) / 100)} />
              </div>

              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "6px" }}>Rotation: {rotation}°</label>
                <input type="range" min={-180} max={180} value={rotation} onChange={e => setRotation(Number(e.target.value))} />
              </div>

              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "6px" }}>Color</label>
                <div style={{ position: "relative" }}>
                  <button onClick={() => setShowPicker(p => !p)} style={{
                    ...
                    <div style={{ width: "20px", height: "20px", borderRadius: "6px", background: color, border: "2px solid rgba(255,255,255,0.2)" }} />
                    <span style={{ color: "rgba(255,255,255,0.7)", fontSize: "13px" }}>{color.toUpperCase()}</span>
                  </button>
                  {showPicker && (
              ...
              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "8px" }}>Position</label>
                <div style={{ display: "flex", gap: "6px" }}>
                  {positions.map(p => (
                    <button key={p.id} onClick={() => setPosition(p.id)} style={{
                      flex: 1, padding: "8px", borderRadius: "10px", fontSize: "11px", cursor: "pointer",
                      background: position === p.id ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.03)",
                      border: `1px solid ${position === p.id ? "rgba(99,102,241,0.4)" : "rgba(255,255,255,0.06)"}`,
                      color: position === p.id ? "#a5b4fc" : "rgba(255,255,255,0.4)",
                    }}>{p.label}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)", display: "block", marginBottom: "8px" }}>Apply to Pages</label>
                <div style={{ display: "flex", gap: "6px" }}>
                  {(["all", "first", "last"] as const).map(p => (
                    <button key={p} onClick={() => setPages(p)} style={{
                      ...
                      background: pages === p ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.03)",
                      border: `1px solid ${pages === p ? "rgba(99,102,241,0.4)" : "rgba(255,255,255,0.06)"}`,
                      color: pages === p ? "#a5b4fc" : "rgba(255,255,255,0.4)",
                    }}>{p}</button>
                  ))}
                </div>
              </div>
